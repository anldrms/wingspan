...
const cells = AIRCRAFT.map(
  (a) =>
    ...
    aircraftToSvg(a, { width: 220, primary: "#0f172a", secondary: "#64748b" }) +
    `<figcaption style="font:12px sans-serif;margin-top:6px">${a.manufacturer} ${a.name}</figcaption></figure>`,
);
writeFileSync(
  "tmp/sheet.html",
  `<!doctype html><body style="margin:0;padding:16px;background:#f3f4f6"><div style="display:grid;grid-template-columns:repeat(6,1fr);gap:12px">${cells.join("")}</div></body>`,
);
